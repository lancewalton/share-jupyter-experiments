"""Phase 0 deliverable: baseline calibration on FTSE, walk-forward.

Reports empirical vs nominal central-interval coverage (the calibration anchor)
and mean CRPS per horizon, plus PIT diagnostics saved to phase1_pit.png.
"""
...
from mc.forecasters import gaussian_ewma, empirical_scaled, bootstrap_iid
from mc.walkforward import evaluate, NOMINAL_INTERVALS

T = 10
MIN_HISTORY = 500
STRIDE = 5  # thin overlapping windows; n_origins reported for context

forecasters = {
    "gaussian_ewma(0.94)": gaussian_ewma(lam=0.94, min_obs=MIN_HISTORY),
    "empirical_scaled(N=250)": empirical_scaled(N=250),
    "bootstrap_iid(N=250)": bootstrap_iid(N=250, n_sims=10_000, seed=0),
    "bootstrap_iid(N=500)": bootstrap_iid(N=500, n_sims=10_000, seed=0),
}
...
def main() -> None:
    ...
    _plot_pit(results)
    print("saved phase1_pit.png")


def _plot_pit(results: dict) -> None:
    n = len(results)
    fig, axes = plt.subplots(2, n, figsize=(4 * n, 7))
    for j, (name, res) in enumerate(results.items()):
        # PIT histogram at horizon 1 and 10 overlaid via reliability curve.
        for h, colour in ((1, "tab:blue"), (10, "tab:red")):
            ...
            # reliability: empirical coverage vs nominal for central intervals.
            axr = axes[1, j]
        ...
        axr.set_title("reliability", fontsize=9)
        axr.set_xlabel("nominal coverage")
        if j == 0:
            axr.set_ylabel("empirical coverage")
        axr.legend(fontsize=7)
    fig.tight_layout()
    fig.savefig("phase1_pit.png", dpi=110)
