"""POOLED-coeff variant: one coefficient set fit on all train halves, applied to all.
...
from mc.forecasters import fhs_ewma, fhs_stack, _expert_var_features
...
RESULTS = f"{SP}/fhs_stack_pooled.csv"
SOURCES = [
...
def main():
    ...
    for src, pat, mr in SOURCES:
        for path in sorted(glob.glob(pat)):
            ...
            all_series.append((src, tk, path, mr))

    # POOLED coefficients: one fit over every series' train half (variance space)
    F, Y = [], []
    for src, tk, path, mr in all_series:
        r = _returns(path, mr)
        if r is None:
            continue
        half = len(r) // 2
    ...
    print(f"pooled coeffs: {np.array2string(coeffs, precision=3)}", flush=True)

    remaining = [s for s in all_series if (s[0], s[1]) not in done]
