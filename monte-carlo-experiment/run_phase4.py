"""Phase 4: vol-adaptive and multi-N combination vs the EWMA benchmark.

Tests whether rescaling a long-window (fat-tailed) bootstrap to the current
volatility regime closes the concentrated 99% tail gap and finally BEATS the
drift-zero EWMA benchmark on CRPS.
"""
...
forecasters = {
    ...
    "vol_scaled(a=1.0)": bootstrap_vol_scaled(N_shape=1000, B_vol=20, alpha=1.0, seed=0),
    "vol_scaled(a=0.5)": bootstrap_vol_scaled(N_shape=1000, B_vol=20, alpha=0.5, seed=0),
    "combine{60,250,1000}": combine_average(
        [bootstrap_iid(N=n, n_sims=10_000, seed=0) for n in (60, 250, 1000)]
    ),
}
...
def _plot(results: dict) -> None:
    ...
    fig.tight_layout()
    fig.savefig("phase4_pit.png", dpi=110)
