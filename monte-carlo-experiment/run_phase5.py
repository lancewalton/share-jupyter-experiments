"""Phase 5: Filtered Historical Simulation vs the benchmark and Phase-4 best.

The question the whole project has been building to: does per-observation vol
standardisation + forward vol propagation close the multi-day 99% tail gap and
beat drift-zero EWMA on CRPS?
"""
...
from mc.forecasters import gaussian_ewma, bootstrap_vol_scaled, fhs_ewma
...
forecasters = {
    ...
    "vol_scaled(a=0.5)": bootstrap_vol_scaled(N_shape=1000, B_vol=20, alpha=0.5, seed=0),
    "fhs_ewma(0.94)": fhs_ewma(lam=0.94, burn=100, n_sims=10_000, seed=0),
    "fhs_ewma(0.97)": fhs_ewma(lam=0.97, burn=100, n_sims=10_000, seed=0),
}
...
def _plot(results: dict) -> None:
    ...
    fig.tight_layout()
    fig.savefig("phase5_pit.png", dpi=110)
